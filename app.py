...
# 頁面排版設定：全寬展開
st.set_page_config(
    page_title="隔日沖主力短空雷達 (流動性風控 × 嚴格千元以下旗艦版)", 
    layout="wide", 
    page_icon="🎯", 
...
with head_col1:
    st.title("🎯 每日隔日沖主力短空雷達 (流動性風控 × 嚴格千元以下旗艦版)")
    st.caption("🔥 鐵律排除「股價 ≥ 1,000 元」千金股與「日均量 < 1,500 張」冷門股，鎖定核心熟悉標的。")
with head_col2:
    st.write("")
...
# 頂部橫向折疊式設定面板
with st.expander("⚙️ 點此展開／收合【篩選條件、風控門檻與分點特性表匯出】", expanded=False):
    ...
    with f_col2:
        min_vol_threshold = st.number_input("最低日均量門檻 (張)：", min_value=1500, max_value=10000, value=1500, step=500)
    with f_col3:
...
# 【純淨底層標的池】：嚴格鎖定「股價 < 1000 元」且「5日均量 >= 1500 張」之優質活躍股
...
def load_radar_market_data():
    ...
    strict_pool = [
        ...
        {"代號": "2344", "名稱": "華邦電", "昨收": 26.50, "昨日鎖碼量": 96500, "券資比": 6.8, "主力分點": [("凱基-台北", 0.155), ("元大-土城永寧", 0.078)]},
    ...
    for item in strict_pool:
        ...
        # 鐵律過濾 1：絕對排除股價 >= 1000 元
        if close_price >= 1000.0:
            continue

        # 鐵律過濾 2：絕對排除 5 日均量 < 1500 張
        if avg_5d_volume < 1500:
            continue
