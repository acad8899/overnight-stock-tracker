...
with head_col1:
    st.title("🎯 每日隔日沖主力短空雷達 (全市場自動掃描版)")
    st.caption("🔥 每日盤後全市場 1,800+ 檔自動掃描：連線台灣證交所 (TWSE) / 櫃買中心 (TPEx) 真實強勢股與主力籌碼。")
with head_col2:
    st.write("")
    if st.button("🔄 立即全市場即時掃描", use_container_width=True):
        st.cache_data.clear()
        st.rerun()
...
# 執行全市場自動掃描
with st.spinner("正在連線台灣證券交易所 (TWSE) 掃描全市場強勢股與主力鎖碼分點..."):
