...
# 項目 2：融資增加用紅色，減少用綠色之 Pandas 樣式著色器 (跨版本通用安全呼叫)
...
def apply_color_styler(styler, func, subset):
    # 兼容 Pandas 2.1 以前的 applymap 與 Pandas 2.1+ 的 map
    if hasattr(styler, "map"):
        return styler.map(func, subset=subset)
    else:
        return styler.applymap(func, subset=subset)

styled_df_all_m = apply_color_styler(
    df_all_m.style,
    style_margin_changes,
    subset=["9/18融資增減(張)", "近10日累計增減(張)"]
).format({
    "現價": "{:.1f}",
    "9/18融資增減(張)": "{:+,d}",
    "融資餘額(張)": "{:,d}",
...
with m_col2:
    ...
    styled_margin_show = apply_color_styler(
        df_margin_show.iloc[::-1].style,
        style_margin_changes,
        subset=["融資增減"]
    ).format({
        "融資買進": "{:,d}",
